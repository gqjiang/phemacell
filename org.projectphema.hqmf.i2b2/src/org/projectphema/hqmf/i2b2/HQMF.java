...
//		return "Stopped server";
 //   }
    ...
    @GET @Path("/test/ihqmf")
    @Produces("text/xml")
    public String doTestIHQMF() {
    	Processors p = null;
		p = Processors.getInstance(); 
		MyProps pr = MyProps.getInstance();
		ByteArrayOutputStream result = new ByteArrayOutputStream();
		StreamSource source = new StreamSource(pr.xslLoc+"/tohqmf.hqmf.xml");
		try {
     ...
     * @throws Exception
     */
    @GET @Path("/test/i2b2query")
    @Produces("text/xml")
    public String doTestI2B2Query() {
    	Processors p = null;
		p = Processors.getInstance(); 
		MyProps pr = MyProps.getInstance();
		ByteArrayOutputStream result = new ByteArrayOutputStream();
		...
		try {
			...
			i2b2source = new StreamSource(new StringReader(result.toString()));


			p.i2b2 = p.tFactory.newTransformer(new StreamSource(MyProps.getInstance().xslLoc+"/IntermediateToI2B2.xsl"));			
			p.i2b2.transform(i2b2source, new StreamResult(i2b2result));

		} catch(TransformerException e) { return ProcessorErrorHandler.lastFatality; }
		return i2b2result.toString();
    }    
